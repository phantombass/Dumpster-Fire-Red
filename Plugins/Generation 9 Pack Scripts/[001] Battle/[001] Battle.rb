...
class Battle::Move
  ...
  def pbCalcTypeModSingle(moveType, defType, user, target)
    ...
    if Effectiveness.ineffective_type?(moveType, defType)
      ...
      if user.hasActiveAbility?(:CORROSION) && defType == :STEEL && pbDamagingMove?
        ret = Effectiveness::NORMAL_EFFECTIVE_MULTIPLIER
      end
    end
    return ret
  ...
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    ...
    if [:SELFDESTRUCT,:EXPLOSION].include?(@id)
      multipliers[:defense_multiplier] *= 0.5
    end
    ...
    if user.hasActiveAbility?([:UNSEENFIST,:PIERCINGDRILL]) && pbContactMove?(user) && 
      (target.effects.include?([PBEffects::Protect,PBEffects::MatBlock,PBEffects::KingsShield,PBEffects::Obstruct,PBEffects::SilkTrap,
        PBEffects::SpikyShield,PBEffects::BanefulBunker,PBEffects::BurningBulwark]) || @priority > 0 && target.pbOwnSide.effects[PBEffects::QuickGuard] || target.pbOwnSide.effects[PBEffects::WideGuard] && user.index != target.index && pbTarget(user).num_targets > 1)
