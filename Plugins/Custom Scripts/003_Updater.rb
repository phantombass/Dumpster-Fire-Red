...
module Dumpster
  ...
  def self.applyPatch(patch, directory)
    msgwindow = pbCreateMessageWindow
    pbMessageDisplay(msgwindow, _INTL("Applying patch...\\wtnp[0]"))
    begin
      gem 'zip'
      gem 'fileutils'
      ...
      pbMessage(_INTL("Unable to apply the patch."))
      raise error
    ensure
      begin; File.delete(patch); rescue; end
      pbMessage(_INTL("Your game has been updated!\nExiting to apply the changes."))
      exit
    end
  end
  ...
  def self.downloadProgress(progress, total, message)
    ...
    if @@download_progress <= percent - 10
      pbMessageDisplay(@@download_message_window, _INTL("\\se[]" + message + " {1}%\\wtnp[0]", percent.to_s), tts: percent == 100)
