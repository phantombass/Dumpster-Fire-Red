...
module Input
  def self.update
    ...
    # if triggerex?(:V) && $game_temp.in_menu == false && $game_temp.message_window_showing == false && $intro != true && $in_battle != true && $game_system.save_disabled != true && $qol_menu != true
    #   if pbConfirmMessage(_INTL("Would you like to save the game?"))
    #     # $game_temp.begun_new_game = false
    #     pbMessage(_INTL("\\PN saved the game!"))
    #     Game.save($stats.save_filename_number)
    #   end
    # end
    if trigger?(Input::AUX2) && $game_temp.in_menu == false && $game_temp.message_window_showing == false && $intro != true && $in_battle != true
      pbQOLMenu
    end
  end
end
