...
class UI::PokemonSummaryVisuals < UI::BaseVisuals
  ...
  PAGE_HANDLERS.add(:stats, {
    :name       => proc { next _INTL("IVs") },
    :order      => 25,
    :icon_index => 1,
    :draw       => [
      :draw_common_page_contents_ivs,
      :draw_ivs,
      :draw_ability
    ]
  })
  ...
  def draw_common_page_contents_ivs
    draw_page_name
    draw_page_icons
    draw_poke_ball
    draw_pokemon_name
  ...
  def draw_hp_ivs
    hp_text = sprintf("%d", @pokemon.iv[:HP])
    draw_number_from_image(@bitmaps[:numbers], hp_text, 182, 366, align: :right)
  end
  ...
  def draw_ivs
    ...
    GameData::Stat.each_main do |stat|
      next if stat.id == :HP
      label = stat.name_brief + " IV"
      draw_text(label, 238, text_y, theme: stat_themes[stat.id])
      draw_text(@pokemon.iv[stat.id], 458, text_y, align: :right, theme: :black)
      text_y += 32
    end
  end
...
UIActionHandlers.add(UI::PokemonSummary::SCREEN_ID, :evolution, {
  :effect => proc { |screen|
    pkmn = GameData::Species.get_species_form(screen.pokemon.species,screen.pokemon.form)
    ...
    if evolution.length == 1
      ...
      case evo_name
        when "Item","HoldItem"
          param = GameData::Item.get(evolution[0][2]).name
        when "HasMove"
          param = GameData::Move.get(evolution[0][2]).name
        else
          param = evolution[0][2]
        end
      pbMessage(_INTL("Evolves into: {1}: {2} {3}",nextevo,evo_name,param))
    elsif evolution == []
      pbMessage(_INTL("This Pokémon does not evolve."))
    else
      evolution.each do |evo|
        nextevo = GameData::Species.get(evo[0]).name
        next if nextevo == "Shedinja"
        ...
        evo_name = GameData::Evolution.get(evo[1]).real_name
        case evo_name
        when "Item","HoldItem","AlternateItem"
          param = GameData::Item.get(evo[2]).name
          evo_name = "Item" if evo_name == "AlternateItem"
        when "HasMove"
          param = GameData::Move.get(evo[2]).name
        when "Ninjask"
          evo_name = "Level"
          param = evo[2]
        else
        ...
        pbMessage(_INTL("Evolves into: #{nextevo}#{form}:\n{1} {2}",evo_name,param))
      end
    end
  }
})
...
MenuHandlers.add(:summary_screen_interact, :evolution, {
  "name"      => _INTL("How to Evolve..."),
  "order"     => 35,
  "condition" => proc { |screen| next !screen.pokemon.egg? && pbCanEvolve?(screen.pokemon) }
})
...
def pbCanEvolve?(pkmn)
  can_evo = (GameData::Species.get(pkmn.species).get_evolutions == [] || (GameData::Species.get_species_form(pkmn.species,pkmn.form).get_evolutions.length > 0 && GameData::Species.get_species_form(pkmn.species,pkmn.form).get_evolutions[0][1] == :None))
  return !can_evo
end